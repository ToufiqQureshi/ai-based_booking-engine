...
async def test_payments_returns_expected_keys(auth_client: AsyncClient):
    """Response must include every field the frontend reads."""
    res = await auth_client.get("/api/v1/payments")
...
async def test_payments_cannot_access_other_hotel_data(auth_client: AsyncClient):
    """Hotel A must never see Hotel B's data."""
    fake_hotel_id = "00000000-0000-0000-0000-000000000000"
    res = await auth_client.get(f"/api/v1/payments?hotel_id={fake_hotel_id}")
    assert res.status_code in (200, 401, 403, 404)

# --- EMPTY STATE ---

async def test_payments_empty_db_no_crash(auth_client: AsyncClient):
