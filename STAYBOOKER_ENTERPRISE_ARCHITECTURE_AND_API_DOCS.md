# STAYBOOKER AI — ENTERPRISE ARCHITECTURE & COMPREHENSIVE API SPECIFICATION

**Document Version:** 3.0-ENTERPRISE  
**System Name:** Staybooker AI Enterprise Suite (Booking Engine, PMS & Channel Manager)  
**Classification:** Confidential / Technical Architecture & API Specification  
**Target Audience:** Engineering Leadership, Solution Architects, DevOps & Security Auditing Teams  

---

## TABLE OF CONTENTS
1. **[SYSTEM OVERVIEW & ENTERPRISE ARCHITECTURE](#1-system-overview--enterprise-architecture)**
   - 1.1 Architectural Principles & Design Patterns
   - 1.2 High-Level System Topology (Mermaid Diagram)
...
2. **[DATABASE SCHEMA & ENTITY RELATIONSHIPS](#2-database-schema--entity-relationships)**
   ...
   - 2.2 Exhaustive Table DDLs, Indexes & Constraints
   - 2.3 Row-Level Security (RLS) & Multi-Tenant Isolation
3. **[SECURITY, AUTHENTICATION & AUTHORIZATION](#3-security-authentication--authorization)**
   - 3.1 OAuth2 / JWT Flow & Supabase Auth Integration
   - 3.2 Role-Based Access Control (RBAC) & Multi-Property Ownership Matrix
...
5. **[FRONTEND SYSTEM ARCHITECTURE & STATE MANAGEMENT](#5-frontend-system-architecture--state-management)**
   - 5.1 Component Hierarchy & Route Structure
   - 5.2 State Management Workflow (TanStack Query & Context API)
   - 5.3 Embedded Iframe Booking Widget & Custom Styling Bridge
6. **[AI CONCIERGE & RAG AUTOMATION GRAPH](#6-ai-concierge--rag-automation-graph)**
   - 6.1 Multi-Agent State Machine (LangGraph)
   - 6.2 Vector Embeddings & RAG Knowledge Retrieval
...
### 1.1 Architectural Principles & Design Patterns
Staybooker AI is constructed on a decoupled, asynchronous micro-service-inspired monolithic architecture. The system separates the operational **Hotelier Property Management Suite (PMS)** from the high-frequency **Direct Guest Booking Engine**, ensuring that administrative heavy querying does not impact public conversion funnels.

The system utilizes:
- **Asynchronous IO (ASGI)**: FastAPI and SQLModel powered by `asyncpg` for non-blocking database transactions.
- **Multi-Tenant Isolation at Database Level**: All operations enforce strict `hotel_id` tenant scoping via PostgreSQL Row-Level Security (RLS) policies.
- **Event-Driven AI Automation**: LangGraph orchestrates complex guest workflows (inquiry handling, special requests, upselling) via deterministic state machines.

```mermaid
graph TD
    subgraph Client Tier
        W1[Embeddable Booking Widget<br>Website Iframe / JS]
        W2[Hotelier PMS Suite<br>React / TypeScript SPA]
        W3[AI Concierge Chatbot<br>Floating Web Chat]
    end

    subgraph Security Gateway
        CF[Cloudflare Edge Gateway<br>WAF, SSL, Rate Limiter]
    end
    ...
    subgraph Persistence & Storage Cluster
        DB[(Supabase PostgreSQL Cluster<br>PgBouncer Connection Pool)]
...
### 2.2 Exhaustive Table DDLs & Indexing Strategies
...
### 3.1 Authentication Flow & Supabase Auth Integration
...
## 5. FRONTEND SYSTEM ARCHITECTURE & STATE MANAGEMENT
...
### 5.2 State Management Workflow (TanStack Query)
