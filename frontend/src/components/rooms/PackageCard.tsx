  ...
  return (
    <Card className="overflow-hidden group hover:shadow-md transition-all duration-200 border border-border bg-background rounded-xl relative">
      <CardHeader className="p-0 relative">
        <div className="h-24 bg-muted/30 flex items-center justify-center relative overflow-hidden border-b border-border">
          <div className="w-12 h-12 rounded-lg bg-background flex items-center justify-center shadow-sm border border-border group-hover:scale-105 transition-transform">
      ...
      <CardContent className="p-5">
        <div className="flex justify-between items-start mb-2">
          <h3 className="text-lg font-bold text-foreground group-hover:text-blue-600 transition-colors">{pkg.name}</h3>
        </div>
        
        <p className="text-xs font-medium text-muted-foreground line-clamp-2 mb-5 min-h-[32px]">
          {pkg.description || 'No description provided for this package.'}
        </p>

        <div className="space-y-3">
          <p className="text-[10px] font-bold text-muted-foreground uppercase tracking-wider">Package Inclusions</p>
          
          <div className="flex flex-wrap gap-1.5">
            {(pkg.package_items || []).slice(0, 3).map((item, idx) => (
              <div key={idx} className="flex items-center gap-1.5 bg-muted/30 border border-border px-2 py-1 rounded-md">
      ...
      <CardFooter className="p-5 pt-0 flex items-center justify-between border-t border-slate-50 mt-2">
        ...
        <div className="flex gap-2 pt-3">
            <Button 
                variant="outline" 
                size="icon" 
                className="h-8 w-8 rounded-lg text-muted-foreground hover:text-blue-600 hover:bg-blue-50 border-border" 
                onClick={() => onEdit(pkg)}
            >
            ...
            <Button 
                variant="outline" 
                size="icon" 
                className="h-8 w-8 rounded-lg text-muted-foreground hover:text-red-600 hover:bg-red-50 border-border" 
