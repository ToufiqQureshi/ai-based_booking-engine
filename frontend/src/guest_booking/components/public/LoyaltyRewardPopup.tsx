...
import { Sparkles, X, Gift, CheckCircle2, ArrowRight, Trophy, Zap, Star, BedDouble, CalendarPlus } from 'lucide-react';
...
interface StayOfferPopupProps {
    ...
    minNights: number;
    onExtend: () => void;     // jump the guest to the date picker
}

export const StayOfferPopup: React.FC<StayOfferPopupProps> = ({
    isOpen, onClose, title, nudgeTitle, nudgeMessage, rewardLabel,
    currentNights, minNights, onExtend,
}) => {
    const progress = minNights > 0
        ? Math.min(100, Math.round((currentNights / minNights) * 100))
        : 0;
    const remaining = Math.max(0, minNights - currentNights);
                    ...
                    >
                        ...
                        <div className="relative p-7 text-center">
                            ...
                            <div className="flex justify-center mb-5">
                                <div className="relative">
                                    <div className="absolute inset-0 bg-primary/25 rounded-full blur-2xl animate-pulse" />
                                    <div className="relative w-[72px] h-[72px] bg-primary text-white rounded-[1.4rem] flex items-center justify-center shadow-lg shadow-primary/30">
                            ...
                            <div className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-primary/10 text-primary text-[11px] font-bold uppercase tracking-wider mb-3">
                                <Star className="w-3 h-3 fill-primary text-primary" /> {title}
                            </div>
                            ...
                            <div className="mb-5">
                                <div className="flex justify-between text-xs text-slate-500 mb-1.5">
                                    <span>{currentNights} / {minNights} nights</span>
                                    <span>{progress}%</span>
                                </div>
                                <div className="w-full bg-slate-100 dark:bg-slate-800 rounded-full h-3 overflow-hidden">
                                    <motion.div
                            ...
                            <div className="bg-primary/5 border border-primary/20 rounded-2xl p-4 mb-5">
                                <p className="text-[10px] font-bold text-primary/60 uppercase tracking-widest mb-1">
                                    Unlock at {minNights} nights
                                </p>
                                <p className="text-base font-black text-primary">{rewardLabel}</p>
                            </div>

                            <Button
                                onClick={onExtend}
                                className="w-full h-12 text-base font-bold rounded-2xl shadow-lg shadow-primary/25 hover:-translate-y-0.5 transition-all"
                            >
                                <CalendarPlus className="mr-2 w-4 h-4" />
                                Add {remaining} Night{remaining === 1 ? '' : 's'}
                            </Button>
                            ...
                            >
                                No thanks, continue with {currentNights} night{currentNights === 1 ? '' : 's'}
                            </button>
                        </div>
                    </motion.div>
                </div>
