...
export function ModernFloatingLayout(props: any) {
    const {
        ...
        // Per-date calendar data passed from BookingWidget
                                ...
                                <Calendar
                                    ...
                                    components={{
                                        DayContent: ({ date }: any) => {
                                            ...
                                            // Use per-date calendarData if available, fallback to startingPrice
                                            const key = `${date.getFullYear()}-${String(date.getMonth()+1).padStart(2,'0')}-${String(date.getDate()).padStart(2,'0')}`;
                                            const dayData = calendarData?.[key];
                                            const isSoldOut = dayData ? !dayData.available : false;
                                            const priceNum = dayData?.min_price ?? null;
                                            const priceStr = priceNum !== null
                                                ? (formatPrice ? formatPrice(priceNum) : fmtFrom(priceNum, config?.currency))
                                                : (!isPast && startingPrice > 0 ? fmtFrom(startingPrice, config?.currency) : null);
                                            return (
                                                    ...
                                                    {!isPast && (
                                                        <span className={`text-[9px] font-semibold leading-none mt-1 ${
                                                            isSoldOut
                                                                ? 'text-red-400'
                                                                : priceStr
                                                                    ? 'text-emerald-600 group-aria-selected:text-white'
                                                                    : 'text-slate-300'
                                                        }`}>
                                                            {isSoldOut ? 'Sold' : priceStr ?? ''}
                                                        </span>
                                                    )}
                                                </div>
                                            );
...
export function BookingComLayout(props: any) {
    ...
    return (
        <>
            {layout === 'booking' && (
                <div className="w-full max-w-5xl mx-auto flex flex-col sm:flex-row items-stretch gap-1 p-1 rounded-sm shadow-md"
                     style={{ backgroundColor: yellowBg }}>
                    
                    {/* Dates */}
                    <div className="flex-[1.5] w-full text-left bg-white rounded-sm min-h-[48px] flex items-center px-3 relative border-[3px] border-transparent focus-within:border-[#febb02] focus-within:ring-2 focus-within:ring-[#febb02]">
                        <Popover open={isCalendarOpen} onOpenChange={setIsCalendarOpen}>
                            <PopoverTrigger asChild>
