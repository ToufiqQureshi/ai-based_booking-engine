...
Expected: visible
Error: strict mode violation: getByText(/Login failed|Invalid email or password|not reachable/i) resolved to 3 elements:
    1) <div class="text-sm font-semibold">Login failed</div> aka getByText('Login failed', { exact: true })
    2) <div class="text-sm opacity-90">Invalid email or password</div> aka getByText('Invalid email or password', { exact: true })
    3) <span role="status" aria-atomic="true" aria-live="assertive">Notification Login failedInvalid email or password</span> aka getByText('Notification Login')
